...
我们首先调用 `clock_set_next_event` 函数，设置下次的中断时间，为当前时间加上 `100000`。接着我们进行以下步骤：

1. `ticks` 自增 1
2. 如果 `ticks` 为100的倍数：
  - 调用 `print_ticks` 打印 `ticks`
  - `num` 自增 1
3. 如果 `num` 增加到 10：
  - 调用 `sbi_shutdown` 关机

需要注意的是，`kern/trap/trapentry.S`中使用了过期的CSR名称`sbadaddr`，我们需要将其更新为新的标准名称`stval`。

#### 定时器中断处理流程：

1. **中断触发**：当发生时钟中断时，跳转到寄存器 `stvec` 保存的地址执行指令（`__alltraps`）
2. **上下文保存**：保存所有的寄存器
3. **参数传递**：执行 `mov a0, sp` 将 `sp` 保存到 `a0` 中
4. **跳转到处理函数**：跳转到 `trap` 函数继续执行
5. **中断分发**：调用 `trap_dispatch` 函数，判断异常类型为中断
6. **中断处理**：跳转到处理函数 `interrupt_handler` 处继续执行
7. **具体处理**：根据 `cause` 的值，跳转到 `IRQ_S_TIMER` 处继续执行
...
**`csrw sscratch, sp `实现的操作**：将 sp 的值赋值给 sscratc

**`csrrw s0, sscratch, x0 `实现的操作**：将 sscratch 赋值给 s0 ，将 sscratch 置0

**目的是**：将**旧SP值**从专用的 `sscratch` 寄存器转移到一个通用的、可被保存的寄存器 `s0` 中。这样，我们后续就可以用 `STORE s0, 2*REGBYTES(sp)` 指令把它保存到内存的 `trapframe` 里。**`sscratch` 为0是判断中断发生于内核态的一个重要标志**，我们将 sscratch 置0，如果后续在中断处理中又发生了嵌套中断，可以通过检查 `sscratch` 是否为0来判断前一个状态是内核态还是用户态。
