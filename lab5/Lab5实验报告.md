...
先在kern_init函数处打一个断点，在指令中随机找到一条访存指令
![[Pasted image 20251216191926.png]]


![[Pasted image 20251216191948.png]]
此时这条指令含义是对地址为sp+8的位置里存入ra寄存器的值
此时对riscv_cpu_get_phys_page_debug函数，这是用于调试目的的辅助函数，用于获取虚拟地址对应的物理页框地址。进行打断点，发现此处传入的addr即为我们的虚拟地址0xffffffffc0200064,通过step指令发现其内部调用了riscv_cpu_mmu_index函数，这个函数的作用用于确定当前 CPU 环境下应该使用哪个 **TLB**  或 **MMU** 上下文。
![[Pasted image 20251216215458.png]]

![[Pasted image 20251216220517.png]]

其中又调用了get_physical_address函数，传入的为虚拟地址0xffffffffc0200064，通过查看该函数的源码，了解到它的作用是：在 TLB Miss 发生后，通过软件模拟 RISC-V 硬件的页表遍历 (Page Table Walk) 机制，将一个虚拟地址 (VA) 翻译成物理地址 (PA)，并检查访问权限。
![[Pasted image 20251216221116.png]]
该函数有以下作用：
- **读取 PTE：** 使用 `ldq_phys` 等 QEMU 函数，通过**物理地址**读取页表项内容。
    
- **指针 vs. 叶节点：**
    
    - 如果 PTE 是**指针**，更新 `base` 并继续下一级循环。
        
    - 如果 PTE 是**叶节点**，跳出循环，计算最终 PA，并检查权限。
        
- **A/D 位更新：** 模拟 RISC-V 硬件，对页表项的 A (Accessed) 位和 D (Dirty) 位进行**原子更新**。
    
- **权限授予：** 根据 PTE 的 `R` (Read), `W` (Write), `X` (Execute) 标志，结合 CPU 的 `mstatus.MXR` (Make Executable Readable) 标志，确定最终的访问权限。
该函数可以分解为四个主要部分：
...
- **特权模式检查：** 如果 CPU 当前处于 **M 模式 (PRV_M)**，或者 **MMU 特性未开启** (`!riscv_feature(env, RISCV_FEATURE_MMU)`)，则执行 **物理寻址 (Bare Mode)**。
    
- **物理寻址：** 此时虚拟地址等于物理地址 (`*physical = addr`)，且拥有完整的读、写、执行权限。
    

### 阶段二：确定 MMU 配置参数

这部分代码从 CPU 状态寄存器中提取出页表翻译所需的关键参数。

1. **确定特权版本：** 检查 RISC-V 特权架构版本（1.10.0 或更早）。
    
2. **提取 `satp` 或 `sptbr`：**
    
    - 读取 `satp` (Supervisor Address Translation and Protection) 寄存器或旧版 `sptbr` 来获取**根页表的物理基地址 (`base`)**。
        
    - 确定虚拟内存模式 (`vm`)，例如 **Sv39**。
        
3. **设置翻译参数：** 根据 `vm` 模式确定页表级数 (`levels`)、每级页表索引的位数 (`ptidxbits`) 和页表项大小 (`ptesize`)。
    
    - **Sv39 模式:** `levels = 3`, `ptidxbits = 9`, `ptesize = 8` (8字节 PTE)。
        

### 阶段三：虚拟地址合法性检查 (Sign Extension)
...
- **规范检查：** RISC-V 要求虚拟地址必须进行**符号扩展**。例如，Sv39 模式只使用 39 位地址，但运行在 64 位 CPU 上。
    
- **目的：** 检查虚拟地址中未使用的最高位是否与第 38 位一致（即，是正确的符号扩展）。如果不一致，则这是一个无效的虚拟地址，返回 `TRANSLATE_FAIL`。
    

### 阶段四：页表遍历核心循环 (Page Table Walk)

这段代码是整个函数的核心，通过一个 `for` 循环模拟逐级页表查找，直到找到一个叶节点 PTE。
...
for (i = 0; i < levels; i++, ptshift -= ptidxbits) {
    ...
    // 2. 计算 PTE 的物理地址 (pte_addr)
    target_ulong pte_addr = base + idx * ptesize;

    // 3. PMP 权限检查 (可选)
    // 4. 读取 PTE (页表项)
    #elif defined(TARGET_RISCV64)
    target_ulong pte = ldq_phys(cs->as, pte_addr); // <--- QEMU 物理访存函数
    
    // 5. 检查有效性 (PTE_V) 和是否为叶节点 (PTE_R/W/X)
    if (!(pte & PTE_V)) { return TRANSLATE_FAIL; /* Invalid PTE */ } 
    else if (!(pte & (PTE_R | PTE_W | PTE_X))) {
        /* Inner PTE, continue walking */
        base = ppn << PGSHIFT; // 更新下一级页表基址
    } 
    // ... 大量权限和保留位检查 ...
    
    // 6. 权限检查失败
    // 如果权限不足（例如，尝试写入只读页），返回 TRANSLATE_FAIL。

    // 7. 叶节点找到 (成功退出)
    else {
        // 8. **更新 A/D (Accessed/Dirty) 位：** 模拟硬件设置 PTE_A 和 PTE_D 标志位。
        // 这涉及到原子操作 (atomic_cmpxchg) 来确保多核环境下的同步。
        // 9. **计算最终物理地址：**
        // *physical = (ppn | (vpn & ((1L << ptshift) - 1))) << PGSHIFT;
        // 10. **设置 TLB 权限：** 根据 PTE 标志设置最终的读/写/执行权限 *prot。
        return TRANSLATE_SUCCESS;
    }
}
```
