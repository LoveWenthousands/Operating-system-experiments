...
- **实验中的知识点**：
    ...
    - 代码中实现了 `struct inode`（抽象索引节点）、`struct file`（文件对象）以及 `inode_ops`（操作函数指针表）。
        
    - `vfs_open`, `vfs_read` 等函数屏蔽了下层是 SFS 文件系统还是设备文件的差异。
        
- **对应的 OS 原理**：
    
    - **文件系统接口 (File System Interface)** 与 **分层架构 (Layered Architecture)**。
        
    - **抽象 (Abstraction)**：操作系统通过 VFS 提供统一的系统调用接口（POSIX 标准），使得上层应用无需关心底层存储设备的具体实现细节。
        
- **理解与关系**：
    
    - VFS 是操作系统内核中的一层抽象层。实验中的 `inode_ops` 利用 C 语言的函数指针模拟了面向对象的**多态**特性。
        
    - **差异**：课本上通常讲的是概念模型，而实验中我们看到了通过“函数指针表”来实现这种分层的具体工程手段。
        

#### 2. 索引节点 (Inode) 与 混合索引分配

- **实验中的知识点**：
    
    - `struct sfs_disk_inode` 中包含了 `direct[]`（直接索引）和 `indirect`（一级间接索引）。
        
    - `sfs_bmap` 函数实现了从逻辑块号到物理块号的映射计算。
        
- **对应的 OS 原理**：
    
    - **文件实现 (File Implementation)** 中的 **索引分配 (Indexed Allocation)**。
        
- **理解与关系**：
    
    - SFS 采用了类似于 UNIX FFS 的**混合索引分配方式**。
        
    - 相比于**连续分配**（容易产生外部碎片，文件不易扩展）和**链式分配**（不支持随机访问），索引分配支持高效的随机访问且没有外部碎片。
        
    - 实验中的实现是对课本理论的简化版（只有一级间接索引），限制了最大文件大小，但原理一致。
        

#### 3. 空闲空间管理 (Free Space Management)

- **实验中的知识点**：
    
    - `kern/fs/sfs/bitmap.c` 实现了位图操作。
        
    - SFS 的磁盘布局中包含 `freemap` 区域。
        
- **对应的 OS 原理**：
    
    - **位图/位向量 (Bit Vector/Bitmap)** 管理法。
        
- **理解与关系**：
    
    - 原理上管理磁盘空闲块有空闲链表法、成组链接法等。
        
    - 本实验采用了**位图法**，因为其实现简单，且容易在磁盘上找到连续的空闲块（虽然 SFS 不强制连续，但连续分配有助于性能），缺点是位图本身可能占用较大的内存。
        

#### 4. 打开文件表 (Open File Table)

- **实验中的知识点**：
    
    - 进程控制块 `proc_struct` 中包含 `files_struct`，其中维护了 `fd_array`（文件描述符数组）。
        
    - `struct file` 结构体中维护了 `pos`（当前读写指针）和 `open_count`。
        
- **对应的 OS 原理**：
    
    - **系统打开文件表 (System-wide Open-file Table)** vs **进程打开文件表 (Per-process Open-file Table)**。
        
- **理解与关系**：
    
    - **关系**：`fd_array` 对应进程打开文件表，`struct inode` 的链表对应系统级的文件信息。
        
    - **差异**：实验中通过 `struct file` 将进程与 inode 联系起来。不同的进程打开同一个文件，会有不同的 `fd` 和 `struct file`（因为读写位置可能不同），但指向同一个 `inode`。这完美复现了 OS 原理中关于文件共享和隔离的机制。
...
- **实验中的知识点**：
    ...
    - `stdin` (键盘) 和 `stdout` (屏幕) 被封装为文件。
        
    - `dev_stdin.c` 中实现了缓冲区和等待队列。
        
- **对应的 OS 原理**：
    
    - **设备独立性 (Device Independence)**：用户程序对待 I/O 设备就像对待文件一样。
        
    - **I/O 缓冲 (Buffering)**：解决 CPU 与外设速度不匹配的问题。
        
- **理解与关系**：
    
    - 实验通过 VFS 机制实现了“一切皆文件”。
        
    - `dev_stdin` 的实现体现了**生产者-消费者模型**：中断处理程序填入字符，用户读进程，取出字符，缓冲区满或空时涉及进程调度。
