...
load_icode(int fd, int argc, char **kargv)
{
    ...
    // (0) current->mm must be empty for exec
    if (current->mm != NULL) {
    ...
    // (3.2) read & handle each program header
    struct proghdr ph;
    ...
    for (uint32_t i = 0; i < phnum; i++) {
        ...
        // (3.4) allocate pages + load file bytes + zero bss
        ...
        while (cur < map_end) {
            ...
            // overlap of [cur, cur+PGSIZE) with file-backed part [ph.p_va, ph.p_va+ph.p_filesz)
    ...
    if ((page = pgdir_alloc_page(mm->pgdir, USTACKTOP - PGSIZE, PTE_USER)) == NULL) {
        ret = -E_NO_MEM;
        goto bad_mmap;
    }

    // (5) install mm into current & switch pagetable
    mm_count_inc(mm);
    current->mm = mm;
    current->pgdir = PADDR(mm->pgdir);
    lsatp(current->pgdir);

    // (6) build user stack: strings + argv[]
    ...
    for (int i = argc - 1; i >= 0; i--) {
        ...
        // ---- FIX: sp is an integer address, cast to (void*) for copy_to_user ----
    ...
    // return to user mode: clear SPP, set SPIE
    tf->status = (read_csr(sstatus) | SSTATUS_SPIE) & ~SSTATUS_SPP;

    return 0;

bad_exec:
    // switch back to kernel pagetable before cleanup
    lsatp(boot_pgdir_pa);
