...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        // LAB6:2312375 (update LAB5 steps)
        /*
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    //设置子进程父进程为当前进程（current），确保进程树关系合法
    proc->parent = current;  
    // make sure current process's wait_state is 0  
    // 确保父进程当前未处于等待状态，避免调度冲突
...
load_icode(unsigned char *binary, size_t size)
{
    ...
    // 设置用户栈顶地址为 USTACKTOP（用户虚拟地址空间的栈顶，定义在 memlayout.h）
    tf->gpr.sp = USTACKTOP;  
    tf->epc = elf->e_entry;  
    // Set SPP to 0 for user mode, set SPIE to 1 to enable interrupts after sret  
    // 设置陷阱帧状态字：SSTATUS_SPP=0（用户模式），SSTATUS_SPIE=1（启用中断）
    //SSTATUS_SPP（位 8）：特权级标识位，0 表示当前特权级为用户态（U-mode），1 表示内核态（S-mode）。
    //SSTATUS_SPIE（位 5）：中断使能标识位，1 表示允许中断，0 表示禁止中断。
